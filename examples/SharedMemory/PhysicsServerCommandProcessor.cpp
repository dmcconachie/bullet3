...
bool PhysicsServerCommandProcessor::processCreateClothPatchObjFileCommand(const struct SharedMemoryCommand& clientCmd, struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient, int bufferSizeInBytes)
{
	...
	// Ensure we have the required components before proceeding
	const CreateClothPatchObjFileArgs& args = clientCmd.m_createClothPatchObjFileArguments;
	btAssert((clientCmd.m_updateFlags & CREATE_CLOTH_PATCH_OBJ_FILE_NAME) != 0);
	b3Assert((clientCmd.m_updateFlags & CREATE_CLOTH_PATCH_OBJ_FILE_CORNERS) != 0);
	b3Assert((clientCmd.m_updateFlags & CREATE_CLOTH_PATCH_OBJ_FILE_NUM_NODES) != 0);
	btAssert(args.m_fileName);

	// Convert the input corner data into bullet format
	const btVector3 corner00(args.m_corner00[0], args.m_corner00[1], args.m_corner00[2]);
	const btVector3 corner10(args.m_corner10[0], args.m_corner10[1], args.m_corner10[2]);
	const btVector3 corner01(args.m_corner01[0], args.m_corner01[1], args.m_corner01[2]);
	const btVector3 corner11(args.m_corner11[0], args.m_corner11[1], args.m_corner11[2]);

	// Create the mesh to be used
	const int nodesX = args.m_numNodesX;
	const int nodesY = args.m_numNodesY;
	// TODO: make clean error reporting
	b3Assert(nodesX > 1);
	b3Assert(nodesY > 1);
	const int totalVertices = nodesX * nodesY;
...
#define VIDX(_x_, _y_) ((_y_)*nodesX + (_x_))
	...
	// First define the position for all vertices
	for (int iy = 0; iy < nodesY; ++iy)
		{
		const btScalar ty = iy / (btScalar)(nodesY - 1);
		const btVector3 py0 = lerp(corner00, corner01, ty);
		const btVector3 py1 = lerp(corner10, corner11, ty);
		for (int ix = 0; ix < nodesX; ++ix)
		{
			const btScalar tx = ix / (btScalar)(nodesX - 1);
			vertices[VIDX(ix, iy)] = lerp(py0, py1, tx);
		}
	}

// Maps (x, y) vertex index into the flattened squares array
#define SIDX(_x_, _y_) ((_y_)*(nodesX-1) + (_x_))

	// Next define the faces as tripples of vertices modelled after btSoftBodyHelpers::createPatch(...)
	const int totalSquares = (nodesX - 1) * (nodesY - 1);
	int triangles[totalSquares][2][3];
	for (int iy = 0; iy < nodesY; ++iy)
	{
		for (int ix = 0; ix < nodesX; ++ix)
		{
			...
			if (mdx && mdy)
			{
				const int square = SIDX(ix, iy);
				if ((ix + iy) & 1)
				{
					triangles[square][0][0] = VIDX(ix    , iy);
					triangles[square][0][1] = VIDX(ix + 1, iy);
					triangles[square][0][2] = VIDX(ix + 1, iy + 1);

					triangles[square][1][0] = VIDX(ix    , iy);
					triangles[square][1][1] = VIDX(ix    , iy + 1);
					triangles[square][1][2] = VIDX(ix + 1, iy + 1);
				}
				else
				{
					triangles[square][0][0] = VIDX(ix    , iy);
					triangles[square][0][1] = VIDX(ix + 1, iy);
					triangles[square][0][2] = VIDX(ix    , iy + 1);

					triangles[square][1][0] = VIDX(ix + 1, iy + 1);
					triangles[square][1][1] = VIDX(ix + 1, iy);
					triangles[square][1][2] = VIDX(ix    , iy + 1);
				}
			}
		}
	}
...
#undef VIDX
	...
	// Write the metadata, vertices, and faces to an OBJ file
	// Assume that any file that exists at the given location should be overwritten
	{
		CommonFileIOInterface* fileIO(m_data->m_pluginManager.getFileIOInterface());
		const int objFileHandle = fileIO->fileOpen(args.m_fileName, "w+");

		// Write a descriptive header
		{
			char header[] = "# Wavefront OBJ File\n"
			                "# Autogenerated by PhysicsServerCommandProcessor::processCreatePatchCommand\n"
			                "# Timestamp ";
			fileIO->fileWrite(objFileHandle, header, strlen(header));
			time_t ltime = time(NULL);
			char buf[27];
			ctime_r(&ltime, buf);
			buf[26] = 0;
			fileIO->fileWrite(objFileHandle, buf, strlen(buf));
			fileIO->fileWrite(objFileHandle, "\n", 1);

			char mtllib[] = "mtllib placeholder.mtl\n";
			char modelName[] = "o ClothPatch\n";
			fileIO->fileWrite(objFileHandle, mtllib, strlen(mtllib));
			fileIO->fileWrite(objFileHandle, modelName, strlen(modelName));
		}
		...
		// Patterned off of cloth_z_up.obj
		char normalMtlShading[] = "vn 0.0000 0.0000 1.0000\n"
		                          "usemtl None\n"
		                          "s off\n";
		fileIO->fileWrite(objFileHandle, normalMtlShading, strlen(normalMtlShading));
		// Write the face data
		for (int i = 0; i < totalSquares; ++i)
		{
			// Note: OBJ files use 1 based indexing
			char buf[128];
			sprintf(buf, "f %d//1 %d//1 %d//1\n", triangles[i][0][0] + 1, triangles[i][0][1] + 1, triangles[i][0][2] + 1);
			buf[26] = 0;
			fileIO->fileWrite(objFileHandle, buf, strlen(buf));
			sprintf(buf, "f %d//1 %d//1 %d//1\n", triangles[i][1][0] + 1, triangles[i][1][1] + 1, triangles[i][1][2] + 1);
			buf[26] = 0;
			fileIO->fileWrite(objFileHandle, buf, strlen(buf));
		}

		// Include a blank line, then close the file
		fileIO->fileWrite(objFileHandle, "\n", 1);
		fileIO->fileClose(objFileHandle);
	}
