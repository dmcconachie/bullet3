...
static int pybullet_internalGetVector3FromSequence(PyObject* seq, int index, double vec[3])
{
	...
	{
		item = PyList_GET_ITEM(seq, index);
		v = pybullet_internalSetVectord(item, vec);
	}
	else
	{
		item = PyTuple_GET_ITEM(seq, index);
...
static int pybullet_internalGetVector4FromSequence(PyObject* seq, int index, double vec[4])
{
	...
	{
		item = PyList_GET_ITEM(seq, index);
		v = pybullet_internalSetVector4d(item, vec);
	}
	else
	{
		item = PyTuple_GET_ITEM(seq, index);
...
static PyObject* pybullet_createClothPatchObjFile(PyObject* self, PyObject* args, PyObject* keywds)
{
	(void)self;
	int physicsClientId = 0;
	...
	// Convert python objects to c objects
	{
		...
		if (!cornersObj || Py_SIZE(cornersObj) != 4)
		{
			PyErr_SetString(SpamError, "Incorrect size for corners object.");
			return NULL;
		}
		pybullet_internalGetVector3FromSequence(cornersObj, 0, corner00);
		pybullet_internalGetVector3FromSequence(cornersObj, 1, corner10);
		pybullet_internalGetVector3FromSequence(cornersObj, 2, corner01);
		pybullet_internalGetVector3FromSequence(cornersObj, 3, corner11);
...
static PyMethodDef SpamMethods[] = {
	...
	{"getBasePositionAndOrientation", (PyCFunction)pybullet_getBasePositionAndOrientation, METH_VARARGS | METH_KEYWORDS,
	...
	{"getBaseVelocity", (PyCFunction)pybullet_getBaseVelocity, METH_VARARGS | METH_KEYWORDS,
	...
	{"calculateInverseKinematics2", (PyCFunction)pybullet_calculateInverseKinematics2, METH_VARARGS | METH_KEYWORDS,
	 "Inverse Kinematics bindings: Given an object id, "
	...
	{"setAdditionalSearchPath", (PyCFunction)pybullet_setAdditionalSearchPath, METH_VARARGS | METH_KEYWORDS,
	 "Set an additional search path, used to load URDF/SDF files."},

	{"getAPIVersion", (PyCFunction)pybullet_getAPIVersion, METH_VARARGS | METH_KEYWORDS,
